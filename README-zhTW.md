...
* 偏愛 `if/unless` 修飾符當你有單行的主體。另一個好的方法是使用控制流程的 `and/or` 。
...
* 如果你覺得是適當的話，使用其他習慣的註釋關鍵字，但記得把它們記錄在專案的 `README` 或類似的地方。
...
* 考慮加入工廠方法來提供額外合理的方式，創造一個特定類別的實體。
...
在本指南所寫的每個東西都不是定案。這只是我渴望想與同樣對Ruby程式設計風格有興趣的大家一起工作，以致於最終我們可以替整個Ruby社群創造一個有益的資源。
