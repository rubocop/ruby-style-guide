...
Avoid comma after the last argument in a block, except in cases where only a single argument is present and its removal would affect functionality.
...
# good - removing the comma here would give a different output
[[1, 2, 3], [4, 5, 6]].map do { |a,| a }
----
