...
Consider using it only when there is a valid reason to restrict the result `true` or `false`. Examples include outputing to a particular format or API like JSON, or as the return value of a `predicate?` method. In these cases, also consider doing a nil check instead: `!something.nil?`.
 ...
 * Be sure to http://blog.marc-andre.ca/2010/11/15/methodmissing-politely/[also define `respond_to_missing?`]
