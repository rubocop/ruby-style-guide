...
For Ruby 2.7 or greater, use `filter_map` instead of `find_all`/`select`/`filter` + `map`.

[source,ruby]
----
# bad
numbers.find_all { |x| x.odd? }.map { |x| x.to_s }
numbers.select { |x| x.odd? }.map { |x| x.to_s }
numbers.filter { |x| x.odd? }.map { |x| x.to_s }

# good
numbers.filter_map { |x| x.to_s if x.odd? }
----
